...
public sealed record GetTradeHistoryQuery(
    Guid? BuyerId,
    Guid? SellerId,
    int Page) : IRequest<Result<IReadOnlyList<TradeSummaryDto>>>;

public class GetTradeHistoryQueryHandler : IRequestHandler<GetTradeHistoryQuery, Result<IReadOnlyList<TradeSummaryDto>>>
{
    ...
    public async Task<Result<IReadOnlyList<TradeSummaryDto>>> Handle(GetTradeHistoryQuery request, CancellationToken ct)
    {
        var page = request.Page <= 0 ? 1 : request.Page;

        var trades = await _tradeRepo.GetHistoryAsync(request.BuyerId, request.SellerId, page, PageSize, ct);

        IReadOnlyList<TradeSummaryDto> result = trades
            .Select(TradeDtoMapper.From)
            .ToList();
        return Result<IReadOnlyList<TradeSummaryDto>>.Ok(result);
    }
}
