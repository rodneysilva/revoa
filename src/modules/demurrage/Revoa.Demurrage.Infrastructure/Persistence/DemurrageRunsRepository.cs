...
public class DemurrageRunsRepository : IDemurrageRunRepository
{
    private readonly IMongoCollection<DemurrageRun> _runs;

    public DemurrageRunsRepository(IMongoDatabase database)
    {
        _runs = database.GetCollection<DemurrageRun>("DemurrageRuns");
    }

    public async Task AddAsync(DemurrageRun run, CancellationToken ct)
    {
        await _runs.InsertOneAsync(run, cancellationToken: ct);
    }
