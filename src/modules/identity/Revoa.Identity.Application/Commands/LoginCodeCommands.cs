...
public sealed record LoginClaims(Guid UserId, string Nome, string Email, bool EmailVerified, bool PhoneVerified);
...
public class LoginConfirmCommandHandler : IRequestHandler<LoginConfirmCommand, Result<LoginClaims>>
{
    ...
    public async Task<Result<LoginClaims>> Handle(LoginConfirmCommand request, CancellationToken ct)
    {
        ...
        await _users.UpdateAsync(user, ct);
        return Result<LoginClaims>.Ok(new LoginClaims(user.Id, user.Nome, user.Email, user.EmailVerified, user.PhoneVerified));
    }
}
