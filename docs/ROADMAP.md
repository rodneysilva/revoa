...
**Módulos:** **Identity** (UF-02/03/04/05) · **Account** (Safe, saldo) · **Token** (faucet R$20→mint; cupom on-chain maxUses) · **BlockchainIndexer** (eventos → read models idempotentes `txHash+logIndex` — **pendente/roadmap**; hoje a chain é consultada via Nethereum).
...
### DoD
- Cadastro funciona com/sem cupom; e-mail + WhatsApp verificados; carteira gerada (EOA managed — ADR-0015); faucet R$20 (+cupom) creditado; saldo visível (consulta direta à chain; Indexer pendente).
...
**Módulos:** **Catalog** (UF-07..11: anúncios kind+VOs, modos, visibilidade, mint, comparativo, metadata MinIO *roadmap*, feed por geolocalização) · **Exchange** (UF-12/13: trocas + estados; UF-14/15: **doação/voluntariado** reusando escrow/voucher com valor 0 + recompensa multi-eixo) · **Community** (UF-18..21: default+user, criador+moderadores, posts recursivos materialized path depth 6, chat SignalR, membership) · **Notifications** (UF-32: SignalR + Web Push).
...
### Entregáveis
**Módulos:** **PricingIntelligence** (UF-28: recálculo sob demanda via endpoint admin (`POST /api/pricing/refresh`; Quartz semanal é roadmap) → API ML + admin seed + comunidade + IPCA/IBGE → Ollama Qwen 7B → ref BRL por categoria, mediana RVM, sugestão justa, cotação `GET /api/pricing/rate`; página de transparência) · **Token** (UF-27: **demurrage** preview/run admin sob demanda — sem job agendado (keeper/scheduler: roadmap); UF-29: **cupom/convite on-chain** CRUD admin + resgate + rate-limit; UF-30: **parâmetros admin** — `DonationReward:BonusRvm`, taxa, demurrage, faucet) · **Reputation** (UF-23: reviews 1–5, níveis/badges, selos 🎁/🤝, pontos de ajuda) · **Moderation** (UF-24/25: árbitro de disputas, moderadores de comunidade, denúncias/bans/auditoria).
...
> domínio + off-chain + on-chain com anvil, verdes no CI), ✅ Estimativa BRL simbólica nos anúncios + redesign landing (feed após hero),
...
### Entregáveis
**Testes E2E (Testcontainers Mongo+anvil + Playwright):** cadastro (sem/com cupom)→faucet→saldo; mint NFT→comprar→atomic swap→release (−2%); P2P *(roadmap — sem endpoint)*; voucher→redeem; voucher expira→reembolso; **doação (fila→curadoria→recompensa)**; voluntariado; disputa→árbitro; comunidade (post recursivo + chat ao vivo); cupom on-chain→mint; demurrage; comparativo; recuperação admin+timelock; **navegação anônima + gate**.
**Observabilidade:** OpenTelemetry + Serilog (tracing API→chain).
...
### DoD
- E2E verde (`workers:1` autenticados); CI verde (5 jobs — `backend` · `frontend` · `contracts` · `onchain` · `e2e`, ver `.github/workflows/ci.yml`); container segue checklist `infra/README.md` (0.0.0.0, X-Forwarded, /health, sem portas host, DB só interna); revoa.me acessível + PWA instalável; revoa.org com transparência.
